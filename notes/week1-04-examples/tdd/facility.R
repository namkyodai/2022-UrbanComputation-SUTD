...
df4<-df3%>%
  select(Year,Facility,NPV)%>%
  filter(!is.na(Year))%>%
  group_by(Facility)

df5<-melt(df4,id=c("Year","Facility"))
df6<-cast(df5,Facility~Year,sum,na.rm = TRUE)
...
yy=y%>%
  select(Facility,immediate,shortterm,mediumterm,longterm,visionary,NPV)%>%
  filter(NPV > 0)

write_xlsx(yy,"../data/summary-facility.csv")
...
#Discipline
cashflow <- ggplot(df3)+
  geom_bar(aes(x = Year, y = NPV/epsilon,fill=Facility),
           stat='identity', na.rm=TRUE)+
  labs(title = "", x = "Year",y = "Peso (1 millions)")+
...
#ggsave("../../pics/cashflow-facility.png",width = 10, height = 6) #This is optional
